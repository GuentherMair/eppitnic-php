 ...
 * `tasks` row (object='registry'); this is what actually reaches the
 * registry once the date arrives.
 *
...
final class DomainReapDeletionsCommand extends Command
{
    ...
    public function run(): int {
        ...
        $rows = R::getAll("SELECT * FROM tasks WHERE object = 'registry' AND active = 1 AND date <= CURRENT_DATE ORDER BY id ASC");
        if ($rows === []) {
