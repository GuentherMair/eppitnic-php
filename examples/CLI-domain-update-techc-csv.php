...
if ( !isset($options['f']) || !isset($options['c']) ) {
  echo "SYNTAX: " . $argv[0] . " -f FILE -c CONTACT[:CONTACT:CONTACT:CONTACT:CONTACT:CONTACT]\n";
  exit(1);
}
...
if (empty($domains)) {
  echo "no domain names found in file '{$options['f']}'\n";
  exit(2);
}
...
if ( ! $session->hello() ) {
  ...
  if ( $session->login() === FALSE ) {
    ...
    foreach ($domains as $name) {
      ...
      if (! is_array($contacts_old))
        $contacts_old = array($contacts_old);
