...
| Method & path | Auth | Notes |
|---|---|---|
| `GET /v1/changelog/{object}/{object_id}` | user | **not scoped at all** — any valid token can read any object's history. `object` is one of `users`\|`contacts`\|`domains`; `data` is the JSON snapshot recorded at that point in time, `action` is `create`\|`update`\|`delete` |
