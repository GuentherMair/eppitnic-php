...
| `GET /v1/domains/{name}` | user | registry-first: live EPP `fetch()`, returned as-is in the `domainToArray()` shape with `"stale": false`. If the registry can't answer — `fetch()` fails **or** the session itself fails — falls back to the local DB row and returns it with `"stale": true`. 404, without asking the registry, for a domain (or pending transfer-in) not of the caller's reseller; otherwise only when neither source has it. This route no longer returns 502 |
...
| `POST /v1/domains/import` | user | body `{"domains": ["a.it", ...]}` — pulls each from the registry into the local DB (idempotent reconciliation, not a create). Response is a per-domain diagnostic object keyed by domain name, each with `domain` and `registrant` (`"found"`/`"not found"`, or for a non-admin `"held by another reseller"` — such a domain is left untouched) and `contact_stored` and `domain_stored` (`"stored"`/`"not stored"`). A step that was never reached reads `"skipped"`, so the first non-`skipped` failure is where the import stopped and why — not a simple success flag, and useful for surfacing partial failures in a bulk-import UI |
